 ...
 *
 * * data-ozi-select → chave única do componente
 * * data-ozi-select-options → conecta ao conteudo
 * * data-ozi-select-multiple → ativa seleção múltipla
 *
 * * data-ozi-select-multiple-group →  ativa seleção múltipla com toggle por grupo
 *
 * * data-ozi-select-submit-name → nome base dos hidden inputs gerados no submit
 *
 * * data-ozi-select-submit-fields → mapeia campos específicos do item selecionado
 *
 * ..md
 * ex:
 * "value:id,label:nome"
 ...
 * texto padrão quando não houver seleção
 *
 ...
 *
 * data-ozi-select-image-dimension →  dimensão da imagem da opção
 * ex:
 * "24px,24px"
 *
 * data-ozi-select-disabled → desabilita o componente
 *
 * data-ozi-select-required → ativa validação obrigatória
 *
 * data-ozi-select-required-message →  mensagem visual de validação

 * ## [4] ESTRUTURA DE CADA OPTION
 *
 * value        →  valor interno da opção
 * label        →  texto padrão da opção
...
(function ($) {
    ...
    function OziSelect(element) {
        ...
        if (!this.key) {
            throw new Error('OziSelect 4.0.0: data-ozi-select é obrigatório.');
        }
    ...
    OziSelect.prototype.loadOptions = function () {
        ...
        // 1) tenta perto do próprio componente
        var $script = this.$root.nextAll(selector).first();

        // 2) tenta no mesmo container pai
        if (!$script.length) {
            $script = this.$root.parent().find(selector).first();
        }
        ...
        if (!$script.length) {
            if (this.zldLog) {
                console.warn('OziSelect 4.0.0: JSON não encontrado para "' + this.key + '".');
            }
            return [];
        }
        ...
        try {
            ...
            console.error('OziSelect 4.0.0: erro ao parsear JSON de "' + this.key + '".', error);
            return [];
        }
    };
    ...
    OziSelect.prototype.buildUI = function () {
        ...
        this.$ui = $('<div>', {
            class: 'ozi-select-ui ozi-select-ui-v400'
        });
        ...
        this.$value = $('<div>', {
            class: 'ozi-select-value'
        });

        var $actions = $('<div>', {
            class: 'ozi-select-actions'
        });
        ...
        this.$dropdown = $('<div>', {
            class: 'ozi-select-dropdown'
        });

        var $searchWrap = $('<div>', {
            class: 'ozi-select-search-wrap'
        });
    ...
    OziSelect.prototype.bindEvents = function () {
        ...
        this.$ui.on('click', '.ozi-select-control', function (e) {
            ...
            if ($(e.target).closest('.ozi-select-clear, .ozi-select-toggle, .ozi-select-tag-remove').length) {
                return;
            }
    ...
    OziSelect.prototype.close = function (focusControl) {
        ...
        if (focusControl && !this.isDisabled()) {
    ...
    OziSelect.prototype.normalizeSubmitMode = function (raw) {
        ...
        if (['legacy', 'value', 'value-label'].indexOf(mode) === -1) {
            return 'value-label';
        }
    ...
    OziSelect.prototype.appendStructuredHiddenField = function (suffix, value) {
        ...
        this.appendHiddenInput(
            this.buildStructuredInputName(suffix),
            value
        );
    };
    ...
    OziSelect.prototype.appendStructuredHiddenInputs = function (item) {
        ...
        var self = this;
        var includeLabel = this.submitMode === 'value-label';

        // value principal
        self.appendStructuredHiddenField('', self.resolveStructuredSubmitValue(item, 'value'));

        // label auxiliar
        if (includeLabel) {
            self.appendStructuredHiddenField('label', self.resolveStructuredSubmitValue(item, 'label'));
        }

        // extras opcionais
        this.submitExtraFields.forEach(function (field) {
            if (field === 'label' && includeLabel) return;

            var value = self.resolveStructuredSubmitValue(item, field);
            self.appendStructuredHiddenField(field, value);
        });
    };
    ...
    OziSelect.prototype.appendTextOrHtml = function ($target, className, htmlValue, textValue, fallbackValue) {
        var hasText = textValue !== undefined && textValue !== null && String(textValue).trim() !== '';
        var hasHtml = htmlValue !== undefined && htmlValue !== null && String(htmlValue).trim() !== '';

        if (hasText || fallbackValue) {
            $target.append(
                $('<div>', { class: className }).text(textValue || fallbackValue || '')
            );
        }
        ...
        if (hasHtml) {
            ...
            this.renderOptionalHtml($htmlNode, htmlValue);
            $target.append($htmlNode);
        }
    };
    ...
    OziSelect.prototype.buildRenderBlocks = function (items) {
        ...
        items.forEach(function (item) {
            var groupName = item && item.oziGroup != null
                ? String(item.oziGroup).trim()
                : '';
    ...
    OziSelect.prototype.renderGroupBlock = function (block) {
        var groupSelected = this.groupToggleEnabled && this.mode === 'multiple'
            ? this.isGroupFullySelected(block.oziGroup, true)
            : false;

        var groupPartial = this.groupToggleEnabled && this.mode === 'multiple'
            ? this.isGroupPartiallySelected(block.oziGroup, true)
            : false;

        var $group = $('<div>', {
            class: 'ozi-select-group',
            'data-ozi-group': block.oziGroup
        });
        ...
        if (this.groupToggleEnabled && this.mode === 'multiple' && !this.isDisabled()) {
            $label.attr('data-ozi-group-toggle', block.oziGroup);
        }
        ...
        block.options.forEach(function (item) {
            $group.append(self.buildOption(item));
        });
    ...
    OziSelect.prototype.getItemsByGroup = function (groupName, onlyVisible) {
        ...
        return this.options.filter(function (item) {
            var sameGroup = String(item.oziGroup || '') === group;
    ...
    OziSelect.prototype.isGroupFullySelected = function (groupName, onlyVisible) {
        ...
        return items.every(function (item) {
            return self.isSelected(item.value);
        });
    };
    ...
    OziSelect.prototype.toggleGroup = function (groupName, onlyVisible) {
        ...
        if (!this.groupToggleEnabled) return;
        ...
        if (shouldSelectAll) {
            items.forEach(function (item) {
                if (!self.isSelected(item.value)) {
                    self.selectedItems.push(item);
                }
            });
        } else {
            var valuesToRemove = items.map(function (item) {
                return String(item.value);
            });

            this.selectedItems = this.selectedItems.filter(function (selected) {
                return valuesToRemove.indexOf(String(selected.value)) === -1;
            });
        }
    ...
    OziSelect.prototype.renderOptions = function (query) {
        ...
        blocks.forEach(function (block) {
            if (block.type === 'option') {
                self.$list.append(self.buildOption(block.item));
                return;
            }

            if (block.type === 'group') {
                self.$list.append(self.renderGroupBlock(block));
            }
        });
    };
    ...
    OziSelect.prototype.buildOption = function (item) {
        ...
        // Se veio HTML completo para a opção, usa ele
        if (item.optionHtml && String(item.optionHtml).trim() !== '') {
            var $custom = $('<div>', {
                class: 'ozi-select-option-custom'
            });
        ...
        var $content = $('<div>', {
            class: 'ozi-select-option-content'
        });
        ...
        var $texts = $('<div>', {
            class: 'ozi-select-option-texts'
        });

        this.appendTextOrHtml(
            $texts,
    ...
    OziSelect.prototype.updateUI = function () {
        ...
        if (this.mode === 'single') {
            ...
            this.selectedItems.forEach(function (item) {
                ...
                if (item.tagHtml && String(item.tagHtml).trim() !== '') {
                    var $tagHtml = $('<span>', {
                        class: 'ozi-select-tag-label'
                    });

                    self.renderOptionalHtml($tagHtml, item.tagHtml);
                    $tag.append($tagHtml);
                } else {
                    $tag.append(
                        $('<span>', {
    ...
    OziSelect.prototype.buildSelectedPreview = function (item) {
        var $content = $('<div>', {
            class: 'ozi-select-value-content'
        });
        ...
        if (item.valueHtml && String(item.valueHtml).trim() !== '') {
            var $valueHtml = $('<div>', {
                class: 'ozi-select-value-label'
            });

            this.renderOptionalHtml($valueHtml, item.valueHtml);
            $content.append($valueHtml);
        } else if (item.labelHtml && String(item.labelHtml).trim() !== '') {
            var $labelHtml = $('<div>', {
                class: 'ozi-select-value-label'
            });

            this.renderOptionalHtml($labelHtml, item.labelHtml);
            $content.append($labelHtml);
        } else {
            $content.append(
                $('<span>', {
                    class: 'ozi-select-value-label'
    ...
    OziSelect.prototype.highlightNext = function () {
        ...
        } else if (!$current.length && $visible.length) {
            this.highlightFirstVisible();
        }
    };
    ...
    OziSelect.prototype.ensureOptionVisible = function ($option) {
        ...
        if (optionTop < listTop) {
            ...
            list.scrollTop(optionBottom - list.innerHeight());
        }
    };
    ...
    OziSelect.prototype.emit = function (eventName) {
        ...
        if (this.$root && this.$root[0] && typeof CustomEvent === 'function') {
            this.$root[0].dispatchEvent(new CustomEvent(eventName, {
                bubbles: true,
                detail: detail
            }));
        }
    };
    OziSelect.prototype.emitChange = function () {
        this.emit('ozi:change');
    };

    OziSelect.prototype.getSelectedItems = function () {
        return this.selectedItems.slice();
    };

    OziSelect.prototype.getValue = function () {
        if (this.mode === 'single') {
            return this.selectedItems[0] ? this.selectedItems[0].value : null;
        }
    ...
    OziSelect.prototype.setValue = function (value) {
        ...
        if (this.mode === 'single') {
            ...
            this.selectedItems = values.map(function (current) {
                return self.findOptionByValue(current);
            }).filter(Boolean);
        }
    ...
    OziSelect.prototype.setDisabled = function (state) {
        ...
        if (this.isDisabledConfig) {
            this.$root.attr('data-ozi-select-disabled', 'disabled');
        } else {
            this.$root.removeAttr('data-ozi-select-disabled');
        }
    ...
    OziSelect.prototype.setRequired = function (state) {
        ...
        if (this.isRequiredConfig) {
            this.$root.attr('data-ozi-select-required', 'required');
        } else {
            this.$root.removeAttr('data-ozi-select-required');
        }
        ...
        $fields.each(function () {
            ...
            if (tag === 'select') {
                if ($field.prop('multiple')) {
                    ...
                    [].concat(selected).forEach(function (value) {
    ...
    OziSelect.prototype.appendRemoteParamsToUrl = function (url, query) {
        ...
        if (this.zldParam) {
            params.push(encodeURIComponent(this.zldParam) + '=' + encodeURIComponent(query));
        }
    ...
    OziSelect.prototype.reconcileSelectedItems = function () {
        ...
        oldSelected.forEach(function (oldItem) {
            ...
            if (fromNewList) {
                nextSelected.push(fromNewList);
                return;
            }

            nextSelected.push(oldItem);
        });
    ...
    OziSelect.prototype.applyRemoteOptions = function (options, query) {
        this.options = this.cloneOptions(options);
        this.writeOptionsScript(this.options);
        this.reconcileSelectedItems();
        this.updateUI();
        this.renderOptions(query || '');
    ...
    OziSelect.prototype.handleSearchInput = function (query) {
        ...
        if (!text.length || text.length < this.zldMin) {
            this.abortRemoteRequest();
            this.resetRemoteOptionsToInitial();
            return;
        }
    ...
    OziSelect.prototype.fetchRemoteOptions = function (query) {
        ...
        if (this.zldLog) {
            console.log('OziSelect 4.0.0 | remote request', {
                key: this.key,
                method: method,
                url: url,
                query: query,
                itemName: this.zldItemName
            });
        }
        ...
        return fetch(url, fetchConfig)
            .then(async function (response) {
                ...
                try {
                    ...
                    if (self.zldLog) {
                        console.error('OziSelect 4.0.0 | resposta JSON inválida', err);
                    }
                    return null;
                }

                if (self.zldLog) {
                    console.log('OziSelect 4.0.0 | remote json', json);
                }
            ...
            .catch(function (err) {
                ...
                if (self.zldLog) {
                    console.error('OziSelect 4.0.0 | erro remoto', err);
                }
        ...
        init: function (selector) {
            ...
            $targets.each(function () {
                ...
                if (existing) {
                    ...
                    if (sameElement && $el.data('ozi-select-initialized')) {
                        return;
                    }

                    // instância antiga órfã: destrói e recria
                    if (!sameElement && !oldStillInDom) {
                        existing.destroy();
                    } else if (!sameElement && oldStillInDom) {
                        // já existe outra instância viva com a mesma key
                        return;
                    }
                }
            ...
            var observer = new MutationObserver(function (mutations) {
                mutations.forEach(function (mutation) {
                    Array.prototype.forEach.call(mutation.addedNodes || [], function (node) {
                        ...
                        if ($node.is('[data-ozi-select]')) {
                            window.OziSelect.init($node);
                            return;
                        }
